...
const ADMIN_EMAILS = ["ks2353013@gmail.com"];
...
app.get("/api/admin/overview", protect, adminOnly, async (req, res) => {
  try {
    const [users, companies, suppliers, leads, tasks, documents, upgradeRequests, leadRequests] = await Promise.all([
      User.countDocuments(),
...
app.put("/api/admin/upgrade-requests/:id", protect, adminOnly, async (req, res) => {
  try {
    const request = await UpgradeRequest.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
app.put("/api/admin/lead-requests/:id", protect, adminOnly, async (req, res) => {
  try {
    const request = await LeadRequest.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json({ success: true, request });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
