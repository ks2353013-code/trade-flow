...
app.post("/api/employees", protect, async (req, res) => {
  try {
    ...
    if (!name || !email || !password) {
      return res.status(400).json({
        success: false,
        message: "Employee name, email and password required",
      });
    }
...
function createCrudRoutes(route, Model, name, limitType) {
  ...
  app.get(`/api/${route}`, protect, async (req, res) => {
    try {
      const items = await Model.find({ companyId: req.companyId }).sort({
        createdAt: -1,
      });
...
app.post("/api/ai/lead-score", protect, (req, res) => {
  ...
  res.json({
    ...
    message:
      score > 85
        ? "High quality verified lead"
        : "Good lead, needs follow-up",
  });
});
...
app.post("/api/outreach/whatsapp", protect, async (req, res) => {
  try {
    ...
    const link = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(
...
app.post("/api/billing/upgrade-demo", protect, async (req, res) => {
  try {
    ...
    res.json({
      ...
      updatedUsers: users.modifiedCount,
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
app.get("/api/dashboard/stats", protect, async (req, res) => {
  try {
    const [users, suppliers, leads, tasks, negotiations, documents] =
      await Promise.all([
...
// ======================
app.get("/suppliers", protect, async (req, res) => {
  try {
    const suppliers = await Supplier.find({ companyId: req.companyId }).sort({
      createdAt: -1,
    });
